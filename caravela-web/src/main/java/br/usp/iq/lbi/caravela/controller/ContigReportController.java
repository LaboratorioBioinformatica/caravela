...
public class ContigReportController {
	...
	public Download taxonOnContigReportByContigId(Long contigId) {
		...
		fileNameStringBuffer.append("/tmp/").append(contig.getSample().getName())
			.append("_").append(contig.getId())
			.append("_").append("taxaOnContig")
	...
	public Download taxonOnContigReport(Long sampleId) {
		...
		List<Contig> contigList = contigDAO.FindByContigBySample(sample, TII_GREATER_OR_EQUALS_THAN,
				NUMBER_OF_FEATURES_GREATER_OR_EQUALS_THAN, GENUS, NUMBER_OF_BOUNDARIES_LESS_OR_EQUALS_THAN,
				INDEX_OF_CONSISTENCY_TAXONOMIC_BY_COUNT_READS_GREATER_OR_EQUALS_THAN,
		...
		try {
			...
			for (Contig contig : contigList) {
				...
				for (TaxonOnContig taxonOnContig : allTaxaOnContig) {
					StringBuilder line = new StringBuilder();
					line.append(contig.getReference()).append(TAB).append(taxonOnContig.getNumberOfReads()).append(TAB)
							.append(decimal.format(taxonOnContig.getCoverage())).append(TAB)
							.append(taxonOnContig.getRank()).append(TAB)
							.append(taxonOnContig.getTaxon().getScientificName()).append(TAB).append(LINE_SEPARATOR);
					fw.write(line.toString());
				}
			}
	...
	private String createHeaderToContigsTaxonomicallyGoodResolved() {
		return new StringBuilder().append("REFERENCE").append(TAB).append("SIZE").append(TAB).append("ICTCR|SP")
				.append(TAB).append("IVCT|SP").append(TAB).append("BORDERS|SP").append(TAB).append("ICTCR|GE")
				.append(TAB).append("IVCT|GE").append(TAB).append("BORDERS|GE").append(TAB).append("ICTCR|FA")
				.append(TAB).append("IVCT|FA").append(TAB).append("BORDERS|FA").append(TAB).append("GTII").append(TAB)
	...
	private String createHeaderTaxaOnContigFromContigsTaxonomicallyGoodResolved() {
		return new StringBuilder().append("REFERENCE").append(TAB).append("NUMBER OF READS").append(TAB)
				.append("COVERAGE").append(TAB).append("RANK").append(TAB).append("SCIENTIFIC NAME").append(TAB)
				.toString();
