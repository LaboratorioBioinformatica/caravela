...
public class TaxonDAOImpl extends DAOImpl<Taxon> implements TaxonDAO {
	...
	public Long count(Sample sample, String scientificName) {
		Query query = entityManager.createQuery("SELECT COUNT(r.id) FROM Read r WHERE r.sample=:sample AND r.taxonomicAssignment.taxon.scientificName=:scientificName", Long.class);
//		select count(*) from sequence s, taxon t where s.sample_id = 1 and s.taxon_id = t.id and scientific_name like '%fusca%'
		query.setParameter("sample", sample)
